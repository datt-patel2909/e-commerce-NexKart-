...
const addproduct = async (req, res) => {
  ...
  // Handle Cloudinary Image Upload if a file is present
  if (req.file) {
    try {
      ...
      return res.status(StatusCodes.INTERNAL_SERVER_ERROR).json({ msg: "Image upload failed", error: uploadError.message || uploadError });
    }
  }
...
const updateproduct = async (req, res) => {
  ...
  // Handle Cloudinary Image Upload if a new file is uploaded during an update
  if (req.file) {
    try {
      const result = await uploadToCloudinary(req.file.buffer);
      req.body.image = result.secure_url;
